...
export default function Calculator() {
  ...
  const handleExamChange = (e) => {
    ...
    if (value > 100) {
      setError(
        `Wynik z ${subjectNames[e.target.name]} nie może przekraczać 100!`
      );
    } else {
      setError("");
  ...
  const handleGradeChange = (e) => {
    ...
    if (numValue < 1 || numValue > 6) {
      setGradeErrors((prev) => ({
        ...prev,
        [name]: `Ocena ${
          subjectNamesGrades[name] || name
        } musi być między 1 a 6!`,
      }));
    } else {
  ...
  const handleCompetitionChange = (e) => {
    ...
    setCompetitionPoints((prev) => {
      ...
      const total = Object.values(updatedPoints).reduce(
        (sum, val) => sum + val,
        0
      );
      ...
      <Card className="w-[1000px] font-bold m-10 p-10 text-sm/8">
        ...
        <CardContent>
          ...
          <label className="flex items-center cursor-pointer text-lg relative">
            ...
            />
            <span className="w-5 h-5 border-2 rounded-full transition-all duration-300 border-gray-400 peer-checked:border-[#008cd8] peer-checked:bg-[#fcf403] hover:bg-[#008cd8]"></span>
          ...
          <label className="flex items-center cursor-pointer text-lg relative">
            <input
              ...
              onChange={handleBonusChange}
            />
            <span className="w-5 h-5 border-2 rounded-full transition-all duration-300 border-gray-400 peer-checked:border-[#008cd8] peer-checked:bg-[#fcf403] hover:bg-blue-[#008cd8]"></span>
          ...
          <Accordion type="single" collapsible className="w-full font-bold">
            <AccordionItem value="competitions">
              <AccordionTrigger className="font-bold text-2xl py-10">
                Konkursy
              </AccordionTrigger>
              <AccordionContent>
                  ...
                  {[
                    ...
                    <label key={`competition1-${index}`} className="block">
                      <input
                        ...
                        className="w-3 h-3 border-2 border-gray-300 rounded-full cursor-pointer peer appearance-none checked:bg-[#fcf403] checked:border-[#008cd8] transition-colors hover:bg-[#008cd8] "
                      />
                      {option.label}
                    </label>
                  ))}
                ...
                <div className="mb-4 leading-7">
                  ...
                  {[
                    ...
                    {
                      label:
                        " tytuł finalisty konkursu przedmiotowego lub artystycznego (10 pkt)",
                      value: 10,
                    },
                    {
                      label: " tytuł laureata turnieju artystycznego (4 pkt)",
                      value: 4,
                    },
                    {
                      label: " tytuł finalisty turnieju artystycznego (3 pkt)",
                      value: 3,
                    },
                  ].map((option, index) => (
                ...
                <div className="mb-4 leading-7">
                  ...
                  {[
                    ...
                    {
                      label:
                        " dwóch lub więcej tytułów finalisty konkursu interdyscyplinarnego (5 pkt)",
                      value: 5,
                    },
                    {
                      label: " tytuł finalisty konkursu przedmiotowego (7 pkt)",
                      value: 7,
                    },
                    {
                      label:
                    ...
                    {
                      label:
                        " tytuł finalisty konkursu interdyscyplinarnego (3 pkt)",
                      value: 3,
                    },
                  ].map((option, index) => (
                ...
                <div className="mb-4 leading-7">
                  <h4 className="font-bold text-xl">
                    Uzyskanie w zawodach wiedzy będących konkursem albo
                ...
                <div>
                  <h4 className="font-bold text-xl">
                    ...
                    wymienione w pkt 1–4, artystycznych lub sportowych,
                    organizowanych przez kuratora oświaty lub inne podmioty
