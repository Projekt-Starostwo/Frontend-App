...
function SchoolTypeCard3({ school, listaKierunkow, typ, schoolDescription, icon, bgColor }) {
    ...
    <div className="rounded-2xl border overflow-hidden bg-background">
      <div className={`${bgColor} p-5 text-white`}>
        ...
        <p className="text-sm text-white/70">
          {listaKierunkow?.length || 0} kierunków
        </p>
      </div>
        ...
        {schoolDescription && (
          <div className="mb-4">
            <p
              ...
              dangerouslySetInnerHTML={{ __html: showFullDescription ? schoolDescription : `${schoolDescription.slice(0, 100)}...` }}
            ></p>
            <p onClick={() => setShowFullDescription((prev) => !prev)} className='underline cursor-pointer pt-2 text-sm text-muted-foreground'>
                  ...
                  return (
                    <Link key={index} href={`/${school.skrot_szkoly}/${slugify(kierunek.nazwa_kierunku)}`}>
                      <div className="flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground p-2 -mx-2 rounded-lg hover:bg-muted">
                        ...
                        <div>
                          <p className="text-left text-wrap">{kierunek.nazwa_kierunku}</p>
                          {typ === 'Branżowa Szkoła I Stopnia' && school.skrot_szkoly === 'chemik' && (
                            <p className="text-left text-wrap text-xs">(kierunek należy do oddziału wielozawodowego)</p>
                          )}
                          {typ === 'Branżowa Szkoła I Stopnia' &&
