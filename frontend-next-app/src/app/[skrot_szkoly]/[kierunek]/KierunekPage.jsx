...
export default async function KierunekPage({ params }) {
  ...
  console.log(justTheUrl, modifiedText);

  return (
    <div className="h-auto w-full flex flex-col justify-start items-center p-5">
      {(kierunekData.error || schoolData.error) && (
        <ErrorPage
          errorMessage={kierunekData.error.message}
          statusCode={kierunekData.error.statusCode}
        >
          <LinkButton
            buttonStyle={"p-0"}
...
function HeadingAndDescription({ kierunek, school }) {
  ...
  const result = findKierunekByName(
    school.rodzaje_szkoly,
